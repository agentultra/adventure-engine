...
exitCurrentRoom exitName = do
  (GameState w _ _ _ _ _) <- lift get
  room <- currentRoom w
...
instance JSON.ToJSON GameState where
  toJSON (GameState world scenes _ _ eventLog rewards) =
    JSON.object
    [ "world" .= world
    , "scenes" .= scenes
    , "events" .= eventLog
...
emitEvent event =
  lift $ modify (\g@(GameState _ _ _ _ eventLog _) -> g { _gameStateEventLog = eventLog ++ [event]})
...
handle' = do
  (GameState world _ input _ _ _) <- lift get
  (v, args) <- parse input
  ...
  case _exitLock exit of
    ...
    Just (Lock keyItems errText successText Locked)
      | playerHasKeyItems (M.elems . _playerInventory $ world) keyItems -> do
        addWorldLogMessage successText
        unlockDoor exitId
        movePlayer exit playerRoom
      | otherwise -> addWorldLogMessage errText
...
handleUse args = do
  ...
  case parseArgs args of
    ...
    Nothing -> addWorldLogMessage "I don't know how to do that..."
  where
...
updateGame gameState =
  case runIdentity . (`runStateT` gameState) . runExceptT $ handle' of
    ...
    (Right _, g@(GameState world' rvs _ _ _ _)) ->
  ...
  where
    ...
    updateGameErrors g@(GameState _ _ _ errs _ _) e
